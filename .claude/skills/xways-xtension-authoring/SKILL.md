...
**Path anchors.** `references/*.md` and `scripts/*.ps1` paths in this skill resolve from the skill's own directory. `docs/...` and `templates/...` paths resolve from the repo root, three levels up from the skill directory (`${CLAUDE_SKILL_DIR}/../../..`) — in a clone that is the repo root; in a plugin install it is the plugin's cached root. The scripts self-resolve their asset paths and can be run from any working directory.

## Hard gates (never violate)

- **Read-only template source.** `templates/x-tensions/` is the pristine template source — never edit, move, or delete it in place; copy a template into the working copy's `x-tensions/xways-<name>/` first (the scaffold script does this). If you create a local `references/` tree for a user-acquired SDK (per `docs/getting-the-sdk.md`), it is read-only and must **never** be committed (copyright).
...
The scripts live in this skill's `scripts/` directory (`.claude/skills/xways-xtension-authoring/scripts/` in a clone). They self-resolve their template/asset sources and can be run from any working directory; scaffold/build output goes into the working copy that contains them.

- **`scripts/new-xtension.ps1`** — copy a starter template (or a locally-available exemplar) into the working copy's `x-tensions/xways-<name>/`, rename the source files to the `xways-<name>` stem, and set the identity constants. Always run with `-DryRun` first to review the planned copies / renames / edits before committing to them.
